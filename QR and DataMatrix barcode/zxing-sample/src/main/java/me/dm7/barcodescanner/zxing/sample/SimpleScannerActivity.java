...
public class SimpleScannerActivity extends BaseScannerActivity implements ZXingScannerView.ResultHandler {
    ...
    public void handleResult(final Result result) {
        ...
        if(ChangeIpAndPort.writeIpAndPort)
        {
            ...
            String stringprot;
            if(scan)
                stringprot = Character.toString(protect[3].charAt(0));
            else
                stringprot = Character.toString(protect[0].charAt(0));
            for (int i = 0; i < protectmas.length; i++) {
                if (stringprot.toLowerCase().indexOf(protectmas[i])!=-1) {
                    endofgame = true;
                    break;
                }
            ...
            if (scan) {
                if (check == 6 && endofgame) {
                    ...
                    mScannerView.resumeCameraPreview(SimpleScannerActivity.this);
                     ...
                     if (endofgame) {
                        ...
                        }).setMessage(msg + "\nШтрихкод выполнен не по шаблону");
                ...
                if (check > 3 && endofgame) {
                    ...
                    String substring = msg.substring(msg.indexOf('+'),msg.indexOf('_',msg.indexOf('+')));
                    msg=msg.replace(substring,"("+substring+")").replaceAll("\\+","");
                    sendMessage();
                    Toast toast = Toast.makeText(getApplicationContext(),
                            msg, Toast.LENGTH_LONG);
                    toast.show();
